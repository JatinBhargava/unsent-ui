...
export default function Friends() {
  ...
  const [messagesByFriend, setMessagesByFriend] = useState<Record<string, string[]>>({});
    ...
    getIncomingRequests(resolvedUserId)
      .then((users) => {
        const entries = users
          ...
          .filter((u): u is FriendEntry => u.uiStatus === "incoming" || u.uiStatus === "accepted");
        setRequests(entries);
      })
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    setMessagesByFriend((prev) => ({
      ...prev,
      [activeFriendId]: [...(prev[activeFriendId] || []), text],
    }));
    setMessageInput("");
  };
  ...
  const activeFriend = friendList.find((f) => f.userId === activeFriendId) || friendList[0] || null;
      ...
      <div className="w-full max-w-5xl">
        ...
        <section className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="rounded-2xl border border-gray-200/80 bg-white/80 p-4 text-center">
            ...
            <p className="mt-1 text-2xl font-semibold text-gray-900">{friendList.length}</p>
          </div>
          <div className="rounded-2xl border border-gray-200/80 bg-white/80 p-4 text-center">
        ...
        <section className="rounded-3xl border border-gray-200/80 bg-[#fffefc] p-5 shadow-[0_10px_22px_rgba(17,24,39,0.06)] ring-1 ring-white/80 sm:p-6">
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            <aside className="rounded-2xl border border-gray-200/80 bg-white p-3 lg:col-span-1">
              <p className="mb-2 text-xs uppercase tracking-wider text-gray-500">Friend List</p>
              {friendList.length === 0 && (
              ...
              <div className="space-y-2">
                {friendList.map((friend) => (
                  ...
                  >
                    {friend.displayName}
                    <span className="ml-1 text-xs opacity-70">@{friend.username}</span>
                  </button>
                ))}
              </div>
            </aside>
            ...
            <div className="rounded-2xl border border-gray-200/80 bg-white p-4 lg:col-span-2">
              <div className="mb-3 flex items-center justify-between">
                <div>
                  <p className="text-lg font-semibold text-gray-900">
                    {activeFriend ? activeFriend.displayName : "No friend selected"}
                  </p>
                  {activeFriend && (
                    <p className="text-xs text-gray-500">@{activeFriend.username}</p>
                  )}
                </div>
                <button
              ...
              <div className="mb-3 h-56 overflow-y-auto rounded-xl border border-gray-200 bg-stone-50 p-3">
                {!activeFriend ? (
                  <p className="text-sm text-gray-500">Add or accept friends to start chatting.</p>
                ) : (messagesByFriend[activeFriend.userId] || []).length === 0 ? (
                  <p className="text-sm text-gray-500">No messages yet. Say hi.</p>
                ) : (
                  <div className="space-y-2">
                    {(messagesByFriend[activeFriend.userId] || []).map((msg, idx) => (
                      <div
                        key={`${activeFriend.userId}-${idx}`}
