...
Notes:
- Vite reads `VITE_*` variables at build time, so a redeploy is required after env changes.
- If Google OAuth is used, add your Vercel domain to allowed redirect/origin settings on the backend/provider side.
